...
func (s *CreateCmdSuite) TestNextSeqVersion() {
	cases := []struct {
		...
		{"Unpadded numeric order", []string{"10_test", "11_test", "9_test"}, 2, "12", nil},
		{"Mixed zero padding", []string{"0010_test", "2_test"}, 3, "011", nil},
		{"Unsorted input", []string{"99_test", "20_test", "3_test"}, 3, "100", nil},
		{"Equivalent padded versions", []string{"002_test", "2_test"}, 3, "003", nil},
		{"Existing zero", []string{"0_test"}, 3, "001", nil},
		{"Numeric maximum exceeds width", []string{"100_test", "9_test"}, 2, "", errors.New("next sequence number 101 too large, at most 2 digits are allowed")},
		{"Earlier malformed filename", []string{"bad", "9_test"}, 2, "", errors.New("malformed migration filename: bad")},
		{"Earlier invalid version", []string{"bad_test", "9_test"}, 2, "", errors.New(`strconv.ParseUint: parsing "bad": invalid syntax`)},
		{"Uint64 maximum", []string{"18446744073709551615_test"}, 20, "", errors.New("next sequence number would overflow uint64")},
		{"Uint64 maximum before smaller version", []string{"18446744073709551615_test", "9_test"}, 20, "", errors.New("next sequence number would overflow uint64")},
		{"Uint64 last valid increment", []string{"18446744073709551614_test"}, 20, "18446744073709551615", nil},
...
func (s *CreateCmdSuite) TestCreateCmd() {
	...
	cases := []struct {
		...
		{"seq unpadded numeric order", nil, "", []string{"9_nine.up.sql", "9_nine.down.sql", "10_ten.up.sql", "10_ten.down.sql"}, []string{"9_nine.up.sql", "9_nine.down.sql", "10_ten.up.sql", "10_ten.down.sql", "11_eleven.up.sql", "11_eleven.down.sql"}, nil, ".", ts, defaultTimeFormat, true, 2, "sql", "eleven"},
		{"seq mixed widths", nil, "", []string{"2_two.sql", "0010_ten.sql"}, []string{"2_two.sql", "0010_ten.sql", "011_eleven.up.sql", "011_eleven.down.sql"}, nil, ".", ts, defaultTimeFormat, true, 3, "sql", "eleven"},
		{"seq maximum exceeds width", nil, "", []string{"9_nine.sql", "100_hundred.sql"}, []string{"9_nine.sql", "100_hundred.sql"}, errors.New("next sequence number 101 too large, at most 2 digits are allowed"), ".", ts, defaultTimeFormat, true, 2, "sql", "next"},
		{"seq uint64 exhausted", nil, "", []string{"18446744073709551615_max.sql"}, []string{"18446744073709551615_max.sql"}, errors.New("next sequence number would overflow uint64"), ".", ts, defaultTimeFormat, true, 20, "sql", "next"},
