...
func (p *Postgres) dropElement(element sqlElement) (err error) {
	...
	case typeSQL:
		// Slightly modified query generated by the `psql /dT` command, to only fetch type names of the current schema.
		// Source: https://github.com/postgres/postgres/blob/REL_14_0/src/bin/psql/describe.c#L717
		query = `
			...
			/* do not include array types */
			AND NOT EXISTS(SELECT 1 FROM pg_catalog.pg_type el WHERE el.oid = t.typelem AND el.typarray = t.oid)
			/* do not include types defined outside current schema */
	...
	if len(itemNames) > 0 {
		...
		for _, n := range itemNames {
			query = `DROP ` + string(element) + ` IF EXISTS ` + pq.QuoteIdentifier(n) + ` CASCADE`
