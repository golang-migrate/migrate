...
func (ch *ClickHouse) SetVersion(version int, dirty bool) error {
	...
	query := "INSERT INTO " + ch.config.DatabaseName + "." + ch.config.MigrationsTable + " (version, dirty, sequence) VALUES (?, ?, ?)"
...
func (ch *ClickHouse) ensureDistributedTable() error {
	baseTableName := ch.config.MigrationsTable + "_base"
	query := fmt.Sprintf(`
	CREATE TABLE IF NOT EXISTS %s.%s ON CLUSTER %s AS %s.%s Engine=Distributed(%s, %s, %s, sequence)`, ch.config.DatabaseName, ch.config.MigrationsTable, ch.config.ClusterName, ch.config.DatabaseName, baseTableName, ch.config.ClusterName, ch.config.DatabaseName, baseTableName)
