...
// the top-level "nodes" section must be kept. io_uring is disabled because Docker's default
// seccomp profile blocks it; Firebolt Core then bypasses the optional consumers that require it.
// The logger_* keys force the engine's logs to stderr (it otherwise writes them to a file inside
// the container), so dktest's LogStdout/LogStderr capture surfaces real startup errors in the CI
// test output.
...
var (
	...
	specs = []dktesting.ContainerSpec{
		{ImageName: "ghcr.io/firebolt-db/engine:dev", Options: opts},
	}
)
